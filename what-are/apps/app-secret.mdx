...
The **appSecret** is your app's private decryption key. It's used to decrypt events that are sent to your app. It's returned as `service_key_pair.private_key` when you create a SERVICE type access key.
...
1. Create a new SERVICE access key with the same permissions (see [Create Access Key](/api-reference/endpoint/access-keys/create))
